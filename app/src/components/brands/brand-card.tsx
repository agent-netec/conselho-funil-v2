...
export function BrandCard({ brand, onEdit, onDelete, delay = 0 }: BrandCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="h-1.5 w-full bg-gradient-to-r from-[#E6B447] to-blue-600 opacity-60 group-hover:opacity-100 transition-opacity" />
      
      <div className="p-6 flex-1 flex flex-col">
        {/* Header */}
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-[#E6B447]/10 to-blue-600/10 border border-white/[0.05] group-hover:border-[#E6B447]/30 transition-all">
              <Building2 className="h-6 w-6 text-[#E6B447] group-hover:text-[#E6B447]/60" />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="text-xl font-bold text-white group-hover:text-[#E6B447] transition-colors truncate">
                  {brand.name}
                </h3>
                <Badge className={cn(
                  "text-[9px] px-1.5 h-4 font-bold border-none",
                  ahiStatus === 'winner' ? "bg-[#E6B447]/20 text-[#E6B447]" : 
        ...
        <div className="space-y-4 mb-8 flex-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-zinc-900/50 border border-white/[0.03]">
              <div className="flex items-center gap-2">
                <Users className="h-3 w-3 text-[#E6B447]/70" />
        ...
        <div className="pt-4 border-t border-white/[0.05] space-y-3">
          <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-zinc-600">
            ...
            <span className={ahiScore >= 80 ? 'text-[#E6B447]' : ahiScore >= 50 ? 'text-amber-500' : 'text-red-500'}>{ahiScore}%</span>
          </div>
          <div className="h-1 w-full bg-zinc-900 rounded-full overflow-hidden">
            <motion.div
              ...
              className={cn(
                'h-full rounded-full',
                ahiScore >= 80 ? 'bg-gradient-to-r from-[#E6B447] to-blue-500' : ahiScore >= 50 ? 'bg-amber-500' : 'bg-red-500'
              )}
            />
          </div>
