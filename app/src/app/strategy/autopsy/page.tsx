...
import { Stethoscope, Activity, Target, AlertTriangle, Search, Loader2, CheckCircle2, XCircle, Info, Library, Save, Trash2, ExternalLink } from "lucide-react";
    ...
    <div className="container mx-auto p-6 space-y-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-zinc-900/50 border border-zinc-800">
          ...
          <TabsTrigger value="library" className="data-[state=active]:bg-blue-500/10 data-[state=active]:text-blue-400">
            <Library className="h-4 w-4 mr-2" />
            Biblioteca
            {caseStudies.length > 0 && (
              <Badge className="ml-2 bg-blue-500/20 text-blue-400 border-none text-[10px]">{caseStudies.length}</Badge>
            )}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="library" className="mt-6">
          {loadingLibrary ? (
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-1 shrink-0">
                        <a href={cs.url} target="_blank" rel="noopener noreferrer" className="p-1 text-zinc-500 hover:text-zinc-300">
                          <ExternalLink className="w-3.5 h-3.5" />
                        </a>
                        <button onClick={() => handleDeleteCaseStudy(cs.id)} className="p-1 text-zinc-500 hover:text-red-400">
                  ...
                  <CardContent className="pt-0 space-y-3">
                    ...
                    {cs.actionableItems.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-[10px] uppercase tracking-wider font-bold text-zinc-500">Ações</span>
                        <ul className="space-y-0.5">
                          ...
                          {cs.actionableItems.length > 3 && (
                            <li className="text-[10px] text-zinc-600">+{cs.actionableItems.length - 3} mais</li>
                          )}
                        </ul>
                      </div>
                    )}
            ...
            <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed border-zinc-800 rounded-2xl bg-zinc-900/20">
              <Library className="w-12 h-12 text-zinc-700 mb-4" />
              <h3 className="text-lg font-medium text-zinc-400">Nenhum estudo de caso</h3>
              <p className="text-sm text-zinc-500 mt-1">Analise páginas ou concorrentes e salve como estudo de caso permanente.</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
