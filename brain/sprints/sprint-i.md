...
| 8 | — | — | Campaign Command Center sem payoff apos conclusao da Linha de Ouro. Todos os 5 stages ficam "APROVADO" mas nao ha: relatorio de conclusao, resumo executivo, proximos passos, exportacao (PDF/brief), celebracao, nem conexao com lancamento real. Real-Time Performance e mock (Issue #5). UX anticlimatica — usuario completa todo o fluxo e "nada acontece". Precisa de sprint dedicado: Campaign Brief, Launch Checklist, Performance Dashboard real. | OBSERVACAO | BACKLOG |
| 9 | — | — | Keywords Miner e ferramenta isolada — minera termos mas nao conecta com o resto do sistema. Falta: (1) keywords correlacionadas/LSI/longtail, (2) salvar keywords no brand para os conselheiros acessarem, (3) alimentar engines de copy/ads/funnel com keywords mineradas, (4) acoes pos-mineracao ("Adicionar ao Brand", "Criar campanha", "Enviar para Conselho de Copy"). Hoje o usuario minera e nao sabe o que fazer com o resultado. Precisa de sprint dedicado para integrar Keywords Miner → Brand → Engines. | OBSERVACAO | BACKLOG |
| 10 | — | — | Spy Agent muito cru e superficial — escaneia URLs de concorrentes mas entrega dados brutos sem analise estrategica. Falta: (1) identificacao de design system (cores, tipografia, espacamento, componentes), (2) deteccao de tech stack, (3) analise qualitativa (o que e bom, o que e ruim, o que emular, o que evitar), (4) racionalizacao do porquê de cada decisao do concorrente, (5) insights acionaveis para o usuario ("Inspirar-se em X", "Evitar Y por Z"), (6) integracao com Brand/Engines para alimentar criativos. Hoje o usuario escaneia e nao sabe o que fazer com o resultado — mesma lacuna do Keywords Miner (#9). Precisa de sprint dedicado: Spy Agent v2 com analise profunda + integracao. | OBSERVACAO | BACKLOG |
| 11 | — | — | Model Comparison Dashboard (`/intelligence/attribution`) — backend real (AttributionEngine com 4 modelos: last_touch, linear, u_shape, time_decay) + hook `useAttributionData` que busca do Firebase (transactions, events, performance_metrics). APIs de ingestao existem: `POST /api/intelligence/events/ingest` (captura UTM), `POST /api/integrations/offline-conversion` (registra vendas), `POST /api/intelligence/attribution/sync` (consolida bridges). Porem dashboard esta VAZIO porque faltam 3 camadas de ingestao: (1) tracking pixel/script nos funnels para capturar UTMs e enviar eventos automaticamente, (2) webhook de pagamento (Stripe/Hotmart/Kiwify) para registrar transacoes, (3) cron jobs para sync periodico dos attribution bridges. A pagina `/integrations` permite conectar Meta Ads (Ad Account ID + token), mas so popula spend — eventos e transacoes dependem de integracao que nao existe. Feature arquiteturalmente solida mas sem pipeline de dados ativo. | OBSERVACAO | BACKLOG |
| 12 | — | — | LTV & Cohort Intelligence (`/intelligence/ltv`) — backend completo e bem arquitetado: CohortDashboard (4 KPIs + graficos + heatmap), cohort-engine.ts, calculator.ts, APIs `/api/intelligence/ltv/cohorts` + 3 predictive engines (churn-predictor, ltv-estimator, audience-forecaster com Gemini narrative) + alert-generator. Busca dados REAIS de 3 collections Firebase (leads, transactions, brands/{id}/performance_metrics). Porem dashboard mostra TUDO ZERADO (ROI 0.00x, LTV R$0, Leads 0, Payback 72 dias hardcoded) porque faltam as mesmas 3 camadas de ingestao da Issue #11: (1) tracking pixel para popular leads com createdAt/brandId, (2) webhook pagamento para popular transactions com amount/status approved, (3) integracao Meta/Google Ads para popular performance_metrics com spend real. Aba "Retencao & Churn" mostra placeholder "disponivel Sprint 22" mas backend de churn JA EXISTE. Modulo 100% ISOLADO — nao alimenta nem e alimentado por Copy/Ads/Design/Chat/Conselheiros. Nao esta mapeado para uso dos conselheiros. Mesmo padrao da Issue #11: arquitetura solida sem pipeline de dados ativo. | OBSERVACAO | BACKLOG |
| 13 | — | — | Jornada do Lead (`/intelligence/journey`) — backend completo: bridge.ts (ingestao com SHA256 lead ID, UTM first/last touch), journey.ts (CRUD leads/events/transactions com PII criptografado), LeadTimeline.tsx (timeline visual), API `/api/intelligence/journey/[leadId]` (busca lead + 100 eventos), API `/api/intelligence/events/ingest` (recebe eventos de tracking). Seguranca solida: PII encrypted, requireBrandAccess, multi-tenant por brandId, Firestore rules role-based. Porem: (1) "Leads Recentes" e PLACEHOLDER sem backend — deveria listar leads recentes da brand, (2) "Heatmap de Conversao" e PLACEHOLDER sem backend — deveria mostrar funil page_view→lead_capture→checkout→purchase com drop-off rates, (3) busca funciona mas nao ha leads para buscar. Modulo e o PONTO DE CONVERGENCIA do pipeline — se dados existissem aqui, alimentaria automaticamente LTV (#12) e Attribution (#11) que usam mesmas collections. Depende das mesmas 3 camadas: tracking pixel, webhook pagamento, ads API sync. | OBSERVACAO | BACKLOG |
| 14 | — | — | Funnel Autopsy (`/strategy/autopsy`) — nome promete autopsia de funil completo (jornada LP→VSL→Checkout→Upsell→Thank You) mas entrega analise de PAGINA UNICA. Engine solido: scrape (Firecrawl→Jina→Readability→Cheerio) + Gemini PRO (temp 0.2) com 10 experts (Carlton, Halbert, Schwartz, Brunson, etc.) + 5 heuristicas (Hook/Story/Offer/Friction/Trust) + health score 0-10 + tech stack detection. Porem: (1) analisa 1 URL, nao sequencia de paginas do funil, (2) nao mapeia passo a passo ate o checkout, (3) relatorios descartaveis (TTL 30 dias) em vez de estudos de caso permanentes, (4) zero conexao com Pipeline de Funis (nao compara "meu funil vs concorrente"), (5) nao alimenta conselheiros nem engines com insights. Spy Agent usa mesmo endpoint mas sofre das mesmas limitacoes (Issue #10). Recomendacao: Nivel 1 (renomear para "Page Forensics"), Nivel 2 (integrar com Pipeline + salvar como estudos de caso), Nivel 3 (crawler multi-pagina para autopsia de funil real). | OBSERVACAO | BACKLOG |
| 15 | — | — | Pipeline de Funis (`/funnels`) — card "engenheiros" mostra Ticket Medio "R$ NaN". Campo `offer.ticket` e tipo `string` nos types mas codigo faz `Number(funnel.context.offer?.ticket \|\| 0)` — se valor salvo nao e numerico (ex: string vazia, texto, formato "1.000,50"), `Number()` retorna NaN. Fix: `(Number(ticket) \|\| 0)` — NaN e falsy, fallback para 0. | BUG | CORRIGIDO |
| 16 | — | — | Offer Lab (`/intelligence/offer-lab`) — Score de Irresistibilidade parece travado em 24 ("Oferta Fraca") mesmo preenchendo todas as 4 etapas. NAO e bug — score e matematicamente correto. Problema e UX enganosa: (1) sliders Hormozi iniciam em 5/5/5/5 (default) que produz base de apenas 4/80 pontos — formula (5x5)/(5+5)=2.5, normalizado 4. Os 4 steps de texto (promessa, stacking, bonus, escassez) contribuem MAXIMO +20 pontos via bonus condicionais, totalizando 24. (2) Usuario foca nos campos de texto achando que sao o driver do score, mas 80% do score vem dos sliders Hormozi que ficam na sidebar sem destaque. (3) Escala inversa de Tempo/Esforco nao e intuitiva (slider alto = score BAIXO). (4) Sem feedback contextual explicando POR QUE o score esta baixo. (5) Para score 70+ precisa: Dream 9-10, Probabilidade 9-10, Tempo 1-2, Esforco 1-2 — mas nada guia o usuario a fazer isso. Modulo funciona tecnicamente (score dinamico, salva no Firebase, alimenta Funnels/Copy via brain context de Brunson/Kennedy), mas UX faz parecer quebrado. | OBSERVACAO | BACKLOG |
| 17 | — | — | Deep Research (`/intelligence/research`) — 6 bugs cascateados encontrados e corrigidos: (17a) cache Firestore sem composite index → try-catch. (17b) GET loadList sem auth header → 401 → adicionado getAuthHeaders. (17c) POST falhava silenciosamente → adicionado error state + feedback visual. (17d) MCP adapters usavam fetch('/api/mcp/execute') URL relativa no server-side → chamada direta APIs. (17e) Pipeline N+1 Gemini calls sequenciais (60-110s) → single-pass synthesis + Firecrawl paralelo (~25s). (17f) Exa sem contents param + DossierGenerator sem fallback JSON → adicionado contents+useAutoprompt e fence stripping. Pipeline agora funcional. **Deep Research v2 documentado como K-4** (inteligencia de audiencia, URLs custom, chat, RAG). | BUG+OBSERVACAO | CORRIGIDO |
...
| 2026-02-15 | Issue #8: Command Center sem payoff pos-conclusao | OBSERVACAO | Sem relatorio, brief, celebracao ou proximos passos — backlog para sprint futuro |
| 2026-02-15 | Issue #9: Keywords Miner isolado do sistema | OBSERVACAO | Falta correlacionadas, salvar no brand, alimentar engines, acoes pos-mineracao |
| 2026-02-15 | Issue #10: Spy Agent muito cru e superficial | OBSERVACAO | Falta design system, tech stack, bom/ruim, emular/evitar, racionalizacao, integracao |
| 2026-02-15 | Issue #11: Attribution Dashboard vazio (sem ingestao) | OBSERVACAO | Backend real mas falta tracking pixel, webhook pagamento, cron sync — backlog |
| 2026-02-15 | Issue #12: LTV & Cohort Intelligence zerado e isolado | OBSERVACAO | Backend completo (cohort + 3 predictive engines) mas sem pipeline de dados ativo, nao integra com engines/conselheiros — backlog |
| 2026-02-15 | Issue #13: Jornada do Lead placeholders + sem dados | OBSERVACAO | Backend/bridge/encryption prontos, Leads Recentes e Heatmap sao placeholders, ponto de convergencia do pipeline de dados — backlog |
| 2026-02-15 | Issue #14: Funnel Autopsy escopo limitado + desconexao | OBSERVACAO | Analisa 1 pagina (nao funil completo), sem estudos de caso permanentes, sem conexao com Pipeline — backlog |
| 2026-02-15 | Issue #15: Pipeline de Funis R$ NaN no Ticket Medio | CORRIGIDO | `(Number(ticket) || 0)` — NaN e falsy, fallback para 0 |
| 2026-02-15 | Issue #16: Offer Lab score parece travado (UX enganosa) | OBSERVACAO | Score correto matematicamente, mas sliders default 5/5/5/5 = base 4/80, texto contribui max +20 = 24. UX nao guia usuario — backlog |
| 2026-02-15 | Issue #17: Deep Research 500 — 2 bugs de codigo | CORRIGIDO | Fix #1: cache try-catch (Firestore index). Fix #2: auth header escapado no MCP relay. Keys ja configuradas na Vercel. Aguarda deploy |
| 2026-02-15 | Issue #17b: Deep Research GET 401 | CORRIGIDO | loadList() fetch sem auth header → adicionado getAuthHeaders(). Commit dfd9e3188 |
| 2026-02-15 | Issue #17c: Deep Research falhas silenciosas | CORRIGIDO | Adicionado error state + feedback visual (brandId vazio, POST fail, dossie failed). Commit 2cfd64ca8 |
| 2026-02-15 | Issue #17d: MCP adapters URL relativa server-side | CORRIGIDO | Exa/Firecrawl usavam fetch('/api/mcp/execute') que falha no servidor. Fix: chamada direta as APIs com process.env keys. Commit 112657385 |
| 2026-02-15 | Issue #17e: Deep Research 504 timeout | CORRIGIDO | Pipeline fazia N+1 Gemini calls sequenciais (60-110s). Fix: single-pass synthesis + Firecrawl paralelo. Commit cbb145e85 + dd3904229 |
| 2026-02-15 | Issue #17f: Dossie vazio (JSON parse fail) + score 0% | CORRIGIDO | Exa sem contents param (snippets vazios). DossierGenerator sem fallback JSON. Fix: Exa contents+useAutoprompt, JSON fence stripping. Commit 3b34ea71b |
| 2026-02-15 | Deep Research v2 documentado como K-4 | PLANEJADO | Audiencia, URLs custom, chat, RAG integration. Ver sprint-k.md K-4 |
