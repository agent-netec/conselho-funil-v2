...
| 17 | — | — | Deep Research (`/intelligence/research`) retorna 500 ao gerar dossie. 2 bugs de codigo encontrados: (1) CRASH: `getCachedResearch()` usa query Firestore com where+where+orderBy que requer composite index — se index nao existe, lanca `FirebaseError` que propaga como 500. Fix: try-catch defensivo no engine (skip cache se index ausente). (2) Auth header do MCP relay (`/api/mcp/execute` L71) com template literal escapado: `` `Bearer \${apiKey}` `` enviava literal "Bearer ${apiKey}" em vez do valor da variavel — Exa rejeitava todas as requests mesmo com key configurada. Fix: removido backslash. Env vars (EXA_API_KEY, FIRECRAWL_API_KEY) JA configuradas na Vercel. Ambos fixes aplicados — aguardando deploy para re-teste. Pipeline funcional (Exa+Firecrawl+Gemini+Brain Schwartz/Brunson) mas isolado do resto do produto. | BUG+OBSERVACAO | CORRIGIDO |
