...
import { Twitter, Linkedin, Instagram, MessageSquare, AtSign } from 'lucide-react';
...
export function InteractionCard({ interaction, isActive, onClick }: InteractionCardProps) {
  ...
  }[interaction.metadata.sentimentLabel];

  return (
    <Card
      className={cn(
        "p-4 cursor-pointer transition-all hover:border-primary/50 mb-2",
      ...
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="outline" className={cn("text-[10px] px-1.5 py-0", SentimentColor)}>
              {interaction.metadata.sentimentLabel.toUpperCase()}
            </Badge>
            {interaction.type === 'dm' && (
