...
1. Load relevant knowledge fragments from `{project-root}/_netecmt/core/testarch/tea-index.csv`:
...
3. Read related NETECMT artifacts (if available):
...
- Traceability Matrix: `_netecmt/output/traceability-matrix.md`
- Test Design: `_netecmt/output/test-design-epic-2.md`
- Test Results: `ci-artifacts/test-report-2025-01-15.xml`
- NFR Assessment: `_netecmt/output/nfr-assessment-release-1.2.md`
```
...
1. **Update workflow status** (if `append_to_history: true`):
   ...
   - Format:
     ...
     - **Document**: [gate-decision-story-1.3.md](_netecmt/output/gate-decision-story-1.3.md)
...
2. **Generate stakeholder notification** (if `notify_stakeholders: true`):
   ...
   Full Report: _netecmt/output/gate-decision-story-1.3.md
   ```
...
## Integration with NETECMT Artifacts
