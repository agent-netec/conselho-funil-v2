<!-- Powered by NETECMT-CORE™ -->

# Test Automation Expansion

**Workflow ID**: `_netecmt/core/testarch/automate`
**Version**: 4.0 (NETECMT v6)
...
1. **NETECMT-Integrated Mode**: Works WITH NETECMT artifacts (story, tech-spec, PRD, test-design) to expand coverage after story implementation
2. **Standalone Mode**: Works WITHOUT NETECMT artifacts - analyzes existing codebase and generates tests independently
...
### Optional (NETECMT-Integrated Mode)
...
**If framework is missing:** HALT with message: "Framework scaffolding required. Run `NETECMT tea *framework` first."
...
1. **Detect Execution Mode**
   ...
   Check if NETECMT artifacts are available:
   - If `{story_file}` variable is set → NETECMT-Integrated Mode
...
2. **Load NETECMT Artifacts (If Available)**

   **NETECMT-Integrated Mode:**
...
6. **Load Knowledge Base Fragments**
   ...
   **Critical:** Consult `{project-root}/_netecmt/core/testarch/tea-index.csv` to load:
...
1. **Determine What Needs Testing**
   ...
   **NETECMT-Integrated Mode (story available):**
...
1. **Create Automation Summary Document**
   ...
   **Coverage Status:**
   ...
   3. Integrate with quality gate: `NETECMT tea *gate`
   ...
   **Coverage Gaps Identified:**
   ...
   {Same structure as NETECMT-Integrated Mode}
   ...
   {Same checklist as NETECMT-Integrated Mode}
   ````
...
- Updates NETECMT status tracking
...
- Works with any project (NETECMT or non-NETECMT)
...
**Mode:** {standalone_mode ? "Standalone" : "NETECMT-Integrated"}
...
4. Integrate with quality gate: `NETECMT tea *gate`
...
- [ ] Execution mode determined (NETECMT-Integrated, Standalone, or Auto-discover)
- [ ] NETECMT artifacts loaded if available (story, tech-spec, test-design, PRD)
