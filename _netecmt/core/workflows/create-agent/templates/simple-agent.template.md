...
agent:
  metadata:
    id: _netecmt/agents/{agent-name}/{agent-name}.md
    name: 'Persona Name'
    title: 'Agent Title'
    icon: 'emoji'
    type: simple
...
- **id**: Final compiled path (`_netecmt/agents/{name}/{name}.md` for standalone)
...
Questions asked during `NETECMT agent-install`:
...
cp /path/to/commit-poet.agent.yaml _netecmt/custom/agents/
