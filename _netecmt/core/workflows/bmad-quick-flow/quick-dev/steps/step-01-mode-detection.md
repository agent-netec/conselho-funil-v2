...
workflow_path: '{project-root}/_netecmt/core/workflows/NETECMT-quick-flow/quick-dev'
...
- `{execution_mode}` - "tech-spec" | "direct" | "execution" (default)
- `{tech_spec_path}` - Path to tech-spec file (if Mode A)
- `{story_pack_path}` - Path to story pack directory (if detected)
...
### 2. Load Project Context & Story Pack

**Story Pack Detection:**
Check if a Story Pack exists for the current context (e.g., `_netecmt/packs/stories/{{story_key}}` or provided via param).
If found:
- Set `{story_pack_path}` = directory path
- Load `scope.yaml`, `story.md`, and `allowed-context.md` from the pack.
- **CRITICAL:** If Story Pack is loaded, BYPASS Step 2.2 loading of global context.

**Global Context (Conditional):**
If `{story_pack_path}` is NOT set, check if `{project_context}` exists (`**/project-context.md`). If found, load it as foundational reference.
...
**[w] Seems bigger than quick-dev** - see what NETECMT Method recommends
**[e] Execute directly**
```
...
**[w] Start NETECMT Method** (recommended)
