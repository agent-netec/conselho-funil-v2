...
<critical>This workflow handles BOTH new projects AND legacy projects following the NETECMT Method</critical>

<workflow>

<step n="1" goal="Scan for existing work">
<output>Welcome to NETECMT Method, {user_name}!</output>
...
To check progress: Load any BMM agent and run /NETECMT:bmm:workflows:workflow-status
...
1. **NETECMT Method** - Full planning for complex projects
...
<output>Based on your project, here are your NETECMT Method planning options:

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

**1. NETECMT Method** 🎯 {{#if recommended}}(RECOMMENDED){{/if}}
...
<ask>Which NETECMT Method approach fits best?
...
<ask if="n">Which NETECMT Method track instead?
...
<output>Your NETECMT workflow path:
...
**Agent:** {{next_agent}}
**Command:** /NETECMT:bmm:workflows:{{next_workflow_id}}

{{#if next_agent not in [analyst, pm]}}
💡 Start new chat with **{{next_agent}}** agent first.
{{/if}}

To check progress: /NETECMT:bmm:workflows:workflow-status
